...
export class WebSerialPort extends AbstructSerialPort{
    ...
    init = async (opt:object) => {
        const option = opt as {updateReq:()=>Promise<void>}
        if (WebSerialPort.isNode) {
            throw(new Error("js-serial-web exected in node environment"))
        } else {
            if (!WebSerialPort.isInitialized) {
                navigator.serial.addEventListener('connect', option.updateReq)
                navigator.serial.addEventListener('disconnect', option.updateReq)
            }
        }
    }
    ...
    finalize = async (opt:object) => {
        ...
        if (WebSerialPort.isNode) {
            ...
            navigator.serial.removeEventListener('connect', option.updateReq)
            navigator.serial.removeEventListener('disconnect', option.updateReq)
        }
    }
}